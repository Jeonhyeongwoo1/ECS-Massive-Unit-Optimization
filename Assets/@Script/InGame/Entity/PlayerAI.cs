...
namespace MewVivor.InGame.Entity
{
    public class PlayerAI : MonoBehaviour
    {
            ...
            while (_aiDirectionLogicCts != null && !_aiDirectionLogicCts.IsCancellationRequested)
            {
                switch (aiDirectionTypeData.aiDirectionType)
                {
                    ...
                    case AIDirectionType.ToMonster:
                        _direction += GetOverlapCircleDirection(aiDirectionTypeData.radius, aiDirectionTypeData.isOpposite, Layer.AttackableLayer,
        ...
        {
            int count = Physics2D.CircleCastNonAlloc(transform.position, radius, _moveComponent.Direction,
            ...
            foreach (AIDirectionTypeData aiDirectionTypeData in _selectedAIBehaviourData.aiDirectionTypeDataList)
            {
                switch (aiDirectionTypeData.aiDirectionType)
                {
                    ...
                    case AIDirectionType.ToMapBoundary:
                        ...
                        Vector2 endPosition = (Vector2)transform.position +((Vector2) _moveComponent.Direction * aiDirectionTypeData.radius);
